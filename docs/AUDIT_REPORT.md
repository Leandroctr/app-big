...
- **M-9 — corrigido:** removida a policy anon `Allow public read app_settings`
  ...
  `source: "database"` e tenant `pwa.app-bigpix.com`.
- **C-2 — corrigido na branch:** `app/page.tsx` passou de
  ...
  same-origin. A mudança remove apenas a capacidade same-origin do iframe e
  preserva scripts e navegação da splash. TypeScript e build passaram; lint passou sem
...
tenant BigPix com o App ID correto e removido ao final.
...
settings reais do tenant BigPix passou. A validação visual e o console de CSP em
...
exclusivamente de `NEXT_PUBLIC_PUBLIC_URL` no servidor; payload do cliente não
  pode trocar o tenant. O BigPix preserva sua gestão administrativa própria em
  `/admin/administradores` e `/api/admin/admins/**`: somente
  `requireSuperAdmin()` libera a tela e as APIs de criação, edição de role,
  ativação e grants. A role `admin` não satisfaz esse guard.
...
`admin_tenant_access`. No BigPix, a matriz A-1/A-3 passou 24/24: os 18 cenários
obrigatórios e seis regressões adicionais. Além da identidade, tenant, sessão,
legacy e cinco guards, ela validou a UI/API de administradores, criação de admin,
edição de grants, recuperação de senha, política de senha e bloqueio da gestão
para `admin`. O teardown terminou com zero usuários, perfis ou grants sintéticos.
...
Este bloco foi propagado semanticamente para o worktree BigPix de segurança, a
partir do HEAD `508d3226af45789aa947e3afd6b39ba556d50947`. Não houve migration,
...
Resultados finais: 17/17 upload, 8/8 CETEC P1, 13/13 auth estático, 25/25 auth
real (incluindo gestão de administradores, upload e teardown), 7/7 A-5 estático
e 13/13 A-5 real.
...
No BigPix passaram: instalação reproduzível com `npm ci --no-audit --no-fund`,
TypeScript, build Next 16.2.11, lint sem erros (somente o warning preexistente de
`formatDimension`), CETEC P1 8/8, auth estático 13/13, A-5 estático 7/7,
...
CETEC P1 8/8, auth 13/13, A-5 7/7, upload-security 22/22, push-hardening 8/8,
schema-baseline 8/8 e política de CI 15/15.

O BigPix mantém suas particularidades já registradas: gestão de administradores
em `/admin/administradores` e `/api/admin/admins/**` sob `requireSuperAdmin()`,
recuperação de senha e a migration `003` exclusiva deste tenant, com o
respectivo rollback. Por isso a suíte de auth tem 13 casos aqui e 10 nos demais
PWAs. Nada disso foi alterado nesta etapa.
