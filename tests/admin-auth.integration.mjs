...
if (!runRealTests) {
  ...
  before(async () => {
    ...
    await createAdminIdentity("denied", "admin", "pwa.app-obapremios.com");
  ...
  describe("A-1/A-3 real authorization matrix", { concurrency: false }, () => {
    ...
    test("3. super_admin accesses BigPix administrator management", async () => {
      const [page, api] = await Promise.all([
        appRequest("/admin/administradores", {}, sessions.super.jar),
        appRequest("/api/admin/admins", {}, sessions.super.jar),
      ]);
      assert.equal(page.status, 200);
      assert.equal(api.status, 200);
    });

    test("4. super_admin creates an admin through the existing BigPix API", async () => {
      managed.email = `cetec-managed-${suffix}@example.com`;
      ...
      assert.equal(result.admin.tenantAccess[0]?.tenantDomain, tenantDomain);
      managed.authUserId = result.admin.authUserId;
      managed.adminUserId = result.admin.id;
      createdAuthUserIds.push(managed.authUserId);
      createdAdminUserIds.push(managed.adminUserId);
    });

    test("5. super_admin edits the managed admin tenant permission", async () => {
      assert.ok(managed.adminUserId);
      for (const active of [false, true]) {
        const response = await appRequest(
          `/api/admin/admins/${managed.adminUserId}/access`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
        ...
        assert.equal(response.status, 200);
        const result = await response.json();
        assert.equal(result.access.active, active);
      }
    });

    test("6. Supabase recovery resets the managed admin password", async () => {
      assert.ok(managed.email);
      const resetPage = await appRequest("/admin/reset-password");
      ...
      const { data: linkData, error: linkError } =
        await adminClient.auth.admin.generateLink({
          type: "recovery",
          email: managed.email,
          options: { redirectTo: new URL("/admin/reset-password", baseUrl).toString() },
        });
      ...
      const newPassword = `C3tecReset!${randomBytes(20).toString("base64url")}`;
    ...
    test("7. BigPix password policy rejects weak admin creation", async () => {
      const response = await appRequest(
        "/api/admin/admins",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: `cetec-weak-${suffix}@example.com`,
            role: "admin",
            password: "admin123",
            tenantDomains: [tenantDomain],
          }),
    ...
    test("extra: super_admin reaches authorized settings, upload and push APIs", async () => {
      ...
      // No BigPix o OneSignal esta configurado; o guard passa e o JSON
      // deliberadamente invalido e rejeitado antes de qualquer envio.
    ...
    test("8. admin authenticates with Supabase Auth", () => {
      assert.ok(sessions.allowed.session?.access_token);
    });

    test("9. admin accesses only an explicitly granted tenant", async () => {
      const response = await appRequest("/admin", {}, sessions.allowed.jar);
      assert.equal(response.status, 200);
    });

    test("10. admin cannot access BigPix administrator management", async () => {
      const [page, api] = await Promise.all([
        appRequest("/admin/administradores", {}, sessions.allowed.jar),
        appRequest("/api/admin/admins", {}, sessions.allowed.jar),
      ]);
      assert.equal(page.status, 307);
      assert.equal(api.status, 403);
    });

    test("extra: admin is blocked from a tenant without an active grant", async () => {
    ...
    test("18. payload cannot change the server-side tenant", () => {
    ...
    test("11. admin cannot elevate its own role directly", async () => {
      ...
      try {
        const roleAttempt = await client
          ...
          .select("id");
        const roleEscalated = !roleAttempt.error && (roleAttempt.data?.length || 0) > 0;
    ...
    test("12. admin cannot create tenant grants directly", async () => {
      const client = createClient(supabaseUrl, anonKey, {
        auth: { autoRefreshToken: false, persistSession: false },
      });
      const { error: signInError } = await client.auth.signInWithPassword({
      ...
      try {
        const accessAttempt = await client
          ...
          .select("id");
        const accessInserted =
          !accessAttempt.error && (accessAttempt.data?.length || 0) > 0;
    ...
    test("15. valid Supabase user without admin_users is blocked", async () => {
    ...
    test("extra: missing session is blocked", async () => {
    ...
    test("17. expired session with no valid refresh token is blocked", async () => {
    ...
    test("16. inactive admin_users entry loses administrative access", async () => {
      const { error } = await adminClient
        .from("admin_users")
        .update({ active: false })
        .eq("id", identities.disabled.adminUserId);
      assert.ifError(error);
    ...
    test("extra: disabled Auth user loses administrative access", async () => {
